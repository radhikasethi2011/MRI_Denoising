...
if __name__ == "__main__":
    ...
    for phase, dataset_opt in opt['datasets'].items():
        ...
        if phase == 'train' and args.phase != 'val':
            ...
            dataset_opt['val_volume_idx'] = 'all'  # Use only first 2 volumes
            dataset_opt['val_slice_idx'] = 'all'  # Use only first 2 slices
    ...
    # Check if the file exists and if we can write to it
    stage_file_path = opt['stage2_file']
    if not os.path.exists(os.path.dirname(stage_file_path)):
        os.makedirs(os.path.dirname(stage_file_path))

    try:
        with open(stage_file_path, 'w+') as f:
            f.write('')  # Try writing an empty string to check write permissions
        logger.info(f"Successfully created and wrote to {stage_file_path}")
    ...
    stage_file = open(stage_file_path, 'w+')
    for _, data in tqdm(enumerate(val_loader)):
        ...
        for t in range(sqrt_alphas_cumprod_prev.shape[0]):  # linear search with early stopping
            ...
            logger.info(f"Sample {idx}, timestep {t}: diff={diff}, min_diff={min_lh}, min_t={min_t}")
        ...
        logger.info(f"Sample {idx}: volume_idx={volume_idx}, slice_idx={slice_idx}, min_t={min_t}")

    stage_file.close()
    logger.info("Stage 2 matching completed")
    print('done!')
